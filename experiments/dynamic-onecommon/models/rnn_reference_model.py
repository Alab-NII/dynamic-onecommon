...
from models.ctx_encoder import StaticContextEncoder
...
class RnnReferenceModel(nn.Module):
    def __init__(self, word_dict, args):
        ...
        self.ctx_encoder = StaticContextEncoder(domain, args)
        ...
        if args.share_attn:
            ...
            self.attn = nn.Sequential(
                nn.Linear(args.nembed_ctx + args.nhid_lang, args.nhid_sel),
                nn.Tanh(),
                nn.Dropout(args.dropout))
    ...
    def reference_resolution(self, ctx_h, outs_emb, ref_inpt):
        ...
        if self.args.share_attn:
            ...
            ref_logit = self.attn(torch.cat([ctx_h, ref_inpt], 3))
        else:
            # reshape ctx_h and ref_inpt
            ref_inpt = ref_inpt.unsqueeze(2).expand(ref_inpt.size(0), ref_inpt.size(1), ctx_h.size(1), ref_inpt.size(2))
            ctx_h = ctx_h.unsqueeze(0).expand(ref_inpt.size(0), ref_inpt.size(1), ref_inpt.size(2), ctx_h.size(-1))

            ref_logit = self.ref_attn(self.attn(torch.cat([ctx_h, ref_inpt], 3)))
    ...
    def selection(self, ctx_h, outs_emb, sel_idx):
        ...
        if self.args.share_attn:
            sel_logit = self.attn(torch.cat([ctx_h, sel_inpt], 2))
        else:
            sel_logit = self.sel_attn(self.attn(torch.cat([ctx_h, sel_inpt], 2)))
    ...
    def forward(self, ctx, inpt, ref_inpt, sel_idx):
        ...
        if self.args.share_attn:
            lang_logit = self.attn(torch.cat([ctx_h_expand, outs_emb_expand], 3))
        else:
            lang_logit = self.lang_attn(self.attn(torch.cat([ctx_h_expand, outs_emb_expand], 3)))
        ...
        outs = self.hid2output(torch.cat([ctx_h_lang, outs_emb], 2))
    ...
    def write(self, ctx_h, lang_h, max_words, temperature,
        ...
        for _ in range(max_words):
            ...
            if self.args.share_attn:
                lang_logit = self.attn(torch.cat([ctx_h, lang_h_expand], 2))
            else:
                lang_logit = self.lang_attn(self.attn(torch.cat([ctx_h, lang_h_expand], 2)))
            ...
            # compute language output
            out_emb = self.hid2output(torch.cat([ctx_h_lang, lang_h], 1))

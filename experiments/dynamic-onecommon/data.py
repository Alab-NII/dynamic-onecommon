...
class Dictionary(object):
    ...
    def from_json_file(file_name, freq_cutoff, init_dict=True):
...
class WordCorpus(object):
    ...
    def _split_into_batches(self, dataset, bsz, shuffle=True, device=None, name="unknown"):
        ...
        # sort by turn length
        chat_agent_ids.sort(key=lambda x: len(dataset[x[0]]["agents"][x[1]]))
        ...
        while i < len(chat_agent_ids):
            ...
            for _ in range(bsz):
                ...
                for turn in range(num_turns):
                    ...
                    # shape: turn -> (bsz, utterances_length, 1)
                    utterances = []
                    for utterance in dataset[chat_id]["agents"][agent_id][turn]["utterances"]:
                        utterances += utterance
                    batch["input"][turn]["utterances"].append(utterances)

                    # shape: turn -> (bsz, 1)
                    batch["input"][turn]["selection_idxs"].append(len(utterances) - 1)
            ...
            # the longest utterances in each turn
            max_utterances_len = [max(batch["input"][turn]["selection_idxs"]) + 1 for turn in range(num_turns)]
            current_bsz = len(batch["meta"]["chat_ids"])

            for turn in range(num_turns):
                # pad all the utterances to match the longest utterances
                for bi in range(current_bsz):
                    stats['n'] += max_utterances_len[turn]
                    stats['nonpadn'] += len(batch["input"][turn]["utterances"][bi])
                    batch["input"][turn]["utterances"][bi] += [pad] * (max_utterances_len[turn] - len(batch["input"][turn]["utterances"][bi]) + 1) # one additional pad
                ...
                utterances_tensor = torch.Tensor(batch["input"][turn]["utterances"]).long()
                batch["input"][turn]["utterances"] = utterances_tensor.narrow(1, 0, utterances_tensor.size(1) - 1)
                batch["target"][turn]["utterances"] = utterances_tensor.narrow(1, 1, utterances_tensor.size(1) - 1)
                ...
                if device is not None:
                    ...
                    batch["input"][turn]["utterances"].to(device)
                    batch["target"][turn]["utterances"].to(device)

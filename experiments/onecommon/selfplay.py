...
def get_agent_type(model):
    if isinstance(model, HierarchicalReferenceModel):
        return HierarchicalAgent
    elif isinstance(model, (RnnReferenceModel)):
        return RnnAgent
    else:
        assert False, 'unknown model type: %s' % (model)
        
def main():
    parser = argparse.ArgumentParser(description='Selfplay script for OneCommon')
    ...
    parser.add_argument('--context_file', type=str, default='shared_5',
        help='context file used in selfplay')
    ...
    # Markable detector arguments
    parser.add_argument('--markable_detector_file', type=str, default="markable_detector.th",
        help='markable detector for visualizing intermediate referents')
    ...
    if os.path.exists(os.path.join(args.model_dir, args.markable_detector_file)):
        markable_detector = utils.load_model(args.model_dir, args.markable_detector_file)
        if args.cuda:
        ...
        markable_detector_corpus = markable_detector.corpus_ty(domain, args.data, train='train_markable.txt', \
            valid='valid_markable.txt', test='test_markable.txt',
    ...
    for seed in seeds:
        ...
        if os.path.exists(os.path.join(args.data, args.context_file + '.json')):

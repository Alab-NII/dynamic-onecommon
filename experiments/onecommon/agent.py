...
class RnnAgent(Agent):
    ...
    def _encode(self, inpt, dictionary):
        encoded = torch.Tensor(dictionary.w2i(inpt)).long()
        return encoded

    def _decode(self, out, dictionary):
        return dictionary.i2w(out.data.cpu())
    ...
    def read(self, inpt):
        ...
        inpt = self._encode(inpt, self.model.word_dict).unsqueeze(1)
    ...
    def write(self, max_words=100):
        ...
        return self._decode(outs.squeeze(1), self.model.word_dict)
...
class HierarchicalAgent(RnnAgent):
    def __init__(self, model, args, name='Alice', train=False):
        super(HierarchicalAgent, self).__init__(model, args, name=name, train=train)
    ...
    def read(self, inpt):
        inpt = ['THEM:'] + inpt
        inpt = self._encode(inpt, self.model.word_dict).unsqueeze(0)
        self.sents.append(inpt)
        self.mem_h = self.model.read(self.ctx_h, inpt, self.mem_h)

    def write(self, max_words=100):
        outs, logprobs, self.mem_h = self.model.write(self.ctx_h, self.mem_h, max_words, self.args.temperature)

        self.logprobs.extend(logprobs)
        self.sents.append(outs)
        
        # remove 'YOU:'
        outs = outs.narrow(1, 1, outs.size(1) - 1)
        return self._decode(outs.squeeze(0), self.model.word_dict)

    def get_attention(self):
        if self.attn_logit is not None:
            prob = F.softmax(self.attn_logit, dim=1).squeeze()
    ...
    def _make_choice_logits(self):
        ...
        logit = self.model.selection(self.ctx_h, self.mem_h)

        return logit

    def _choose(self, sample=False, logit=None):
        if not logit:
            logit = self._make_choice_logits()

        prob = F.softmax(logit, dim=1)
        if sample:
            idx = prob.multinomial(1).detach()
            logprob = F.log_softmax(logit, dim=1).gather(1, idx)
        else:
            _, idx = prob.max(1, keepdim=True)
            logprob = None

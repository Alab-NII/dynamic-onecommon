...
class RnnReferenceModel(nn.Module):
    ...
    def initialize_from(self, pretrained_model):
        ...
        if not self.args.share_attn:
            self.sel_attn = copy.deepcopy(pretrained_model.sel_attn)
            self.ref_attn = copy.deepcopy(pretrained_model.sel_attn)
    ...
    def reference_resolution(self, ctx_h, outs_emb, ref_inpt):
        ...
        ref_logit = self.ref_attn(torch.cat([ctx_h, ref_inpt], 3))

        ref_logit = torch.transpose(ref_logit, 0, 1).contiguous()
    ...
    def selection(self, ctx_h, outs_emb, sel_idx):
        ...
        sel_logit = self.sel_attn(torch.cat([ctx_h, sel_inpt], 2))
    ...
    def forward(self, ctx, inpt, ref_inpt, sel_idx):
        ...
        lang_logit = self.lang_attn(torch.cat([ctx_h_expand, outs_emb_expand], 3))
    ...
    def write(self, ctx_h, lang_h, max_words, temperature,
        ...
        for _ in range(max_words):
            ...
            lang_logit = self.lang_attn(torch.cat([ctx_h, lang_h_expand], 2))

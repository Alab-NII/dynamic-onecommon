...
class DatabaseManager(object):
    ...
    def init_database(cls, db_file):
        ...
        c.execute(
            ...
            partner_id text, scenario_id text, agent_index integer, selected_index integer, chat_id text, tutorial integer)'''
        )
        c.execute('''CREATE TABLE mturk_task (name text, mturk_code text, chat_id text, hit_id text, assignment_id text)''')

        c.execute(
            '''CREATE TABLE event (chat_id text, action text, agent integer, time text, data text, turn integer, start_time text, metadata text)'''
        )
        c.execute(
            '''CREATE TABLE chat (chat_id text, scenario_id text, outcome integer, agent_ids text, agent_types text,
            start_time text)'''
        )
        c.execute(
...
class Backend(object):
    ...
    def __init__(self, params, schema, scenario_db, systems, sessions, controller_map, num_chats_per_scenario, messages=Messages, active_system=None, active_scenario=None):
        ...
        self.scenario_int_id = {sid: i for i, sid in enumerate(scenario_db.scenarios_map.keys())}
        self.schema = schema
    ...
    def display_received_event(self, event):
        ...
        if event.action == 'message':
            ...
            message = format_message("Turn {} success! Click forward to move to the next turn.".format(int(event.turn) + 1), True)
            turn_success = True
        else:
            message = None
        data = {'status': status}
        data['turn_success'] = turn_success
    ...
    def _update_user(self, cursor, userid, **kwargs):
        ...
        """
        import inspect
        print(userid, set_string, values)
    ...
    def _end_chat(self, cursor, userid, successful_turns):
        ...
        self.update_chat_reward(cursor, controller.get_chat_id(), successful_turns)
        _update_scenario_db()
    ...
    def _get_user_info(self, cursor, userid, assumed_status=None):
        u = self._get_user_info_unchecked(cursor, userid)
        #print(userid, u.status, u.connected_status)
    ...
    def add_event_to_db(self, chat_id, event):
        def _create_row(chat_id, event):
            ...
            return chat_id, event.action, event.agent, event.time, data, event.turn, event.start_time, json.dumps(event.metadata)
    ...
    def attempt_join_chat(self, userid):
        def _init_controller(my_index, partner_type, scenario, chat_id):
            my_session = self.systems[HumanSystem.name()].new_session(scenario, my_index)
            partner_session = self.systems[partner_type].new_session(scenario, 1 - my_index)
        ...
        def _choose_scenario_and_partner_type(cursor):
            ...
            #print(len(active_scenarios.keys()))
            ...
            p = np.random.choice(list(active_scenarios[sid]))
            return self.scenario_db.get(sid), p
        ...
        try:
            with self.conn:
                ...
                if partner_type == HumanSystem.name():
                    ...
                    if my_index == 0:
                        self.add_chat_to_db(chat_id, scenario_id, userid, partner_type, HumanSystem.name(), partner_type)
                    else:
                        self.add_chat_to_db(chat_id, scenario_id, partner_type, userid, partner_type, HumanSystem.name())
    ...
    def end_chat_and_finish(self, cursor, userid, successful_turns=-1, message=None):
        self._end_chat(cursor, userid, successful_turns)
        self.user_finished(cursor, userid, message)

    def timeout_chat_and_skip_survey(self, cursor, userid, current_turn, message=None, partner_id=None):
        if message is None:
            message = self.messages.ChatExpired
        self._end_chat(cursor, userid, current_turn)
        self.user_finished(cursor, userid, message)
        #self._end_chat(cursor, userid)
        #self.skip_survey_and_finish(cursor, userid, message)

    def timeout_chat_and_finish(self, cursor, userid, message=None, partner_id=None, current_turn=0):
        if message is None:
            message = self.messages.ChatExpired
        self.end_chat_and_finish(cursor, userid, current_turn, message)
        if partner_id is not None and not self.is_user_partner_bot(cursor, userid):
            self.user_finished(cursor, partner_id, message)

    def end_chat_and_redirect(self, cursor, userid, current_turn=-1, message=None):
        self._end_chat(cursor, userid, current_turn)
        self._update_user(cursor, userid,
    ...
    """
    def end_chat_and_transition_to_waiting(self, cursor, userid, current_turn, message, partner_id=None):
        self._end_chat(cursor, userid, current_turn)
        self._update_user(cursor, userid,
    ...
    def initialize_status(self, userid):
        with self.conn:
            cursor = self.conn.cursor()
            self._update_user(cursor, userid,
                              status=Status.Waiting,
    ...
    def check_game_over_and_transition(self, cursor, userid, partner_id, turn_over, turn_success, current_turn):
        game_over = (current_turn >= 5) or (turn_over and not turn_success)
        if game_over:
            user_info = self._get_user_info_unchecked(cursor, userid)
            _update_scenario_db(user_info.chat_id, user_info.partner_type)

            successful_turns = current_turn if turn_success else current_turn - 1
            msg, partner_msg = self.get_completion_messages(userid, successful_turns)
            self.end_chat_and_finish(cursor, userid, current_turn, message=msg)
    ...
    def check_turn_over_and_success(self, userid):
        controller = self.controller_map[userid]
        return controller.turn_over_and_success()
    ...
    def create_user_if_not_exists(self, username):
        with self.conn:
            ...
            cursor.execute('''INSERT OR IGNORE INTO active_user VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)''',
                           (username, Status.Waiting, now, 0, now, "", "", "", "", -1, -1, "", 0))
    ...
    def get_completion_messages(self, userid, successful_turns):
        """
        Returns:
            my_message, partner_message
        """
        if successful_turns < 2:
            msg = "You need to succeed at least 2 turns to submit your HIT. Please try again!"
        else:
            msg = self.messages.ChatCompleted.format(successful_turns=successful_turns)
        ...
        try:
            with self.conn:
                ...
                num_seconds = (self.config["status_params"]["finished"]["num_seconds"] +
                               u.status_timestamp) - current_timestamp_in_seconds()
                completed, outcome = _is_chat_complete(cursor, u.chat_id)
                if from_mturk:
                ...
                return FinishedState(Markup(u.message), num_seconds, mturk_code), u.chat_id, completed, outcome
    ...
    def is_chat_valid(self, userid, current_turn):
        try:
            with self.conn:
                cursor = self.conn.cursor()
                try:
                ...
                if not self.is_user_partner_bot(cursor, userid):
                    try:
                        ...
                        self.end_chat_and_redirect(cursor, userid, current_turn=current_turn, message=self.messages.PartnerLeftRoom)
                        return False, False
                    except StatusTimeoutException:
                        self.timeout_chat_and_finish(cursor, userid,
                                                     message=self.messages.ChatExpired + " " + self.messages.HITCompletionWarning,
                                                     partner_id=u.partner_id,
                                                     current_turn=current_turn)
                        # self.end_chat_and_transition_to_waiting(cursor, userid, current_turn=current_turn, message=self.messages.ChatExpired)
                        return False, False
                    except ConnectionTimeoutException:
                ...
                turn_over, turn_success, current_turn = self.check_turn_over_and_success(userid)

                if self.check_game_over_and_transition(cursor, userid, u.partner_id, turn_over, turn_success, current_turn):
    ...
    def send(self, userid, event, update_connected_status=True):
        session = self._get_session(userid)
        session.enqueue(event)
        controller = self.controller_map[userid]
        if update_connected_status:
            with self.conn:
                cursor = self.conn.cursor()
                self._update_user(cursor, userid, connected_status=1)
        if controller is None:
    ...
    def update_chat_reward(self, cursor, chat_id, outcome):
        cursor.execute('''UPDATE chat SET outcome=? WHERE chat_id=?''', (outcome, chat_id))
...
#######################################
    ...
    def disconnect_all_users(self):
        cursor = self.conn.cursor()
        cursor.execute("SELECT name FROM active_user WHERE connected_status=1")
        connected_users = [r[0] for r in cursor.fetchall()]
        for user in connected_users:
            self.disconnect(user)

    def retrieve_all_users(self):
        cursor = self.conn.cursor()
        cursor.execute("SELECT name FROM active_user")

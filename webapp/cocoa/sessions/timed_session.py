...
class TimedSessionWrapper(Session):
    ...
    def __init__(self, session):
        ...
        # JoinEvent
        init_event = Event.JoinEvent(session.my_index,
                                      session.model.name,
                                      str(time.time()),
                                      0)
    ...
    def send(self, turn):
        ...
        if len(self.queued_event) == 0:
            self.queued_event.append(self.session.send(turn))
        ...
        if self.last_message_timestamp + delay > time.time():
            ...
            if event.action == 'message' and self.start_typing is False and \
                ...
                return Event.TypingEvent(self.agent, 'started', str(time.time()), turn)
            else:
                return None
        else:
            if event.action == 'message' and self.start_typing is True:
                self.start_typing = False
                return Event.TypingEvent(self.agent, 'stopped', str(time.time()), turn)
            elif event.action == 'join':

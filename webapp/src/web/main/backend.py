...
class DatabaseManager(BaseDatabaseManager):
    ...
    def add_survey_table(cls, cursor):
        cursor.execute(
            ...
            cooperative integer, obligation integer, comments text)''')
    ...
    def add_scenarios(self, scenario_db, systems, update=False):
        ...
        c = conn.cursor()
        for scenario_id, scenario in scenario_db.scenarios_map.items():
            sid = scenario.uuid
...
class Backend(BaseBackend):
    ...
    def select(self, userid, idx, turn):
        try:
            with self.conn:
    ...
    def should_reject_chat(self, userid, agent_idx):
        with self.conn:
            ...
            num_utterances = sum([1 if event.action == 'message' else 0 for event in ex.events])
            total_utterance_len = sum([len(event.data) if event.action == 'message' else 0 for event in ex.events])
            if num_utterances < 4 or total_utterance_len <= 10:
                return True
            return False
    ...
    def check_game_over_and_transition(self, cursor, userid, partner_id, turn_over, turn_success, current_turn):
        ...
        if game_over:
            ...
            self.end_chat_and_finish(cursor, userid, successful_turns, message=msg)
    ...
    def finish_tutorial(self, userid):
        try:
            with self.conn:
                cursor = self.conn.cursor()
                cursor.execute("UPDATE active_user SET tutorial=1 WHERE name=?", (userid,))

        except sqlite3.IntegrityError:
            print("WARNING: Rolled back transaction")


    def check_tutorial(self, userid):
        try:
            with self.conn:
                cursor = self.conn.cursor()
                cursor.execute("SELECT tutorial FROM active_user WHERE name=?", (userid,))

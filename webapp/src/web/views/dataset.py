...
def visualize():
    ...
    if not request.args.get('chat_id'):
        outcomes = []
        chat_ids = []
        for chat_id in accepted_transcripts.keys():
            outcome = int(accepted_transcripts[chat_id]["outcome"])
            if outcome >= 0:
        ...
        return render_template('simple_chat_list.html',
                                ...
                                base_url = app.config["base_url"] + '{}/dataset?chat_id='.format(app.config["prefix"]),
                                )
    else:
